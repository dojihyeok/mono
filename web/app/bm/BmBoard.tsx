...
// ─────────────────────────────────────────────
// v1.1 — Evidence / Status / Compliance Badge Vocabulary
// (BM 개발 개선 요청서 v1.1 §13, §6.5, §26)
// ─────────────────────────────────────────────
type EvidenceType = 'paid' | 'repeat' | 'pilot' | 'loi' | 'interview' | 'survey' | 'hypothesis' | 'tech_review' | 'poc_planned' | 'partnered';
type ProductStatusTag = 'live' | 'validating' | 'pilot' | 'prototype' | 'tech_review' | 'poc_ready' | 'poc_planned' | 'partnered' | 'enterprise_pilot' | 'roadmap';
type ComplianceStatusTag = 'ready' | 'report_required' | 'license_gate' | 'partner_required' | 'legal_review' | 'blocked';

interface BadgeStyle { label: string; color: string; bg: string; border: string; }

const EVIDENCE_BADGE: Record<EvidenceType, BadgeStyle> = {
  paid: { label: 'PAID', color: '#166534', bg: '#f0fdf4', border: '#bbf7d0' },
  repeat: { label: 'REPEAT', color: '#065f46', bg: '#ecfdf5', border: '#6ee7b7' },
  pilot: { label: 'PILOT', color: '#0e7490', bg: '#ecfeff', border: '#a5f3fc' },
  loi: { label: 'LOI', color: '#7c3aed', bg: '#f5f3ff', border: '#ddd6fe' },
  interview: { label: 'INTERVIEW', color: '#b45309', bg: '#fffbeb', border: '#fde68a' },
  survey: { label: 'SURVEY', color: '#1d4ed8', bg: '#eff6ff', border: '#bfdbfe' },
  ...
  poc_planned: { label: 'POC PLANNED', color: '#c2410c', bg: '#fff7ed', border: '#fed7aa' },
  partnered: { label: 'PARTNERED', color: '#065f46', bg: '#ecfdf5', border: '#6ee7b7' },
};

const PRODUCT_STATUS_BADGE: Record<ProductStatusTag, BadgeStyle> = {
  live: { label: 'LIVE', color: '#166534', bg: '#f0fdf4', border: '#bbf7d0' },
  validating: { label: 'VALIDATING', color: '#1d4ed8', bg: '#eff6ff', border: '#bfdbfe' },
  pilot: { label: 'PILOT', color: '#0e7490', bg: '#ecfeff', border: '#a5f3fc' },
  prototype: { label: 'PROTOTYPE', color: '#7c3aed', bg: '#f5f3ff', border: '#ddd6fe' },
  tech_review: { label: 'TECH REVIEW', color: '#9333ea', bg: '#faf5ff', border: '#e9d5ff' },
  poc_ready: { label: 'POC READY', color: '#0369a1', bg: '#f0f9ff', border: '#bae6fd' },
  poc_planned: { label: 'POC PLANNED', color: '#c2410c', bg: '#fff7ed', border: '#fed7aa' },
  partnered: { label: 'PARTNERED', color: '#065f46', bg: '#ecfdf5', border: '#6ee7b7' },
  enterprise_pilot: { label: 'ENTERPRISE PILOT', color: '#1e3a8a', bg: '#eef2ff', border: '#c7d2fe' },
  roadmap: { label: 'ROADMAP', color: '#64748b', bg: '#f8fafc', border: '#e2e8f0' },
};

const COMPLIANCE_BADGE: Record<ComplianceStatusTag, BadgeStyle> = {
  ready: { label: 'READY', color: '#166534', bg: '#f0fdf4', border: '#bbf7d0' },
  report_required: { label: 'REPORT REQUIRED', color: '#b45309', bg: '#fffbeb', border: '#fde68a' },
  license_gate: { label: 'LICENSE GATE', color: '#b91c1c', bg: '#fef2f2', border: '#fecaca' },
  partner_required: { label: 'PARTNER REQUIRED', color: '#7c3aed', bg: '#f5f3ff', border: '#ddd6fe' },
  legal_review: { label: 'LEGAL REVIEW', color: '#9333ea', bg: '#faf5ff', border: '#e9d5ff' },
  blocked: { label: 'BLOCKED', color: '#f8fafc', bg: '#0f172a', border: '#0f172a' },
};

function Badge({ badge }: { badge: BadgeStyle }) {
  return (
    <span style={{ fontSize: 10.5, fontWeight: 900, color: badge.color, background: badge.bg, border: `1px solid ${badge.border}`, padding: '3px 9px', borderRadius: 6, letterSpacing: '0.02em', whiteSpace: 'nowrap', display: 'inline-block' }}>
      {badge.label}
    </span>
  );
}
...
// ─────────────────────────────────────────────
// v1.2 — Active BM Hypotheses / Experiment Board / Decision Log 데이터 모델
// (MONO 내부 BM 검증 페이지 개발 요청서 v1.2 §5, §7, §9, §11, §14)
// ─────────────────────────────────────────────
...
const HYPOTHESIS_STATUS_BADGE: Record<string, BadgeStyle> = {
  ...
  P1_NEXT: { label: 'P1 NEXT', color: '#1d4ed8', bg: '#eff6ff', border: '#bfdbfe' },
  P2_LATER: { label: 'P2 LATER', color: '#64748b', bg: '#f8fafc', border: '#e2e8f0' },
  HOLD: { label: 'HOLD', color: '#b45309', bg: '#fffbeb', border: '#fde68a' },
  KILL: { label: 'KILL', color: '#f8fafc', bg: '#0f172a', border: '#0f172a' },
};

const EXPERIMENT_STAGES = ['BACKLOG', 'READY', 'RUNNING', 'ANALYZING', 'COMPLETED'] as const;

const DECISION_BADGE: Record<string, BadgeStyle> = {
  KEEP: { label: 'KEEP', color: '#166534', bg: '#f0fdf4', border: '#bbf7d0' },
  CHANGE: { label: 'CHANGE', color: '#1d4ed8', bg: '#eff6ff', border: '#bfdbfe' },
  HOLD: { label: 'HOLD', color: '#b45309', bg: '#fffbeb', border: '#fde68a' },
  KILL: { label: 'KILL', color: '#f8fafc', bg: '#0f172a', border: '#0f172a' },
};

interface BmUnitEconomics {
  price?: string; discount?: string; directCost?: string; pgCost?: string; salesCost?: string;
  cac?: string; grossMargin?: string; mrr?: string; churnRate?: string; expectedLtv?: string; paybackPeriod?: string;
}

interface BmHypothesisData {
  id: string;
  name: string;
  ...
  dueDate: string | null;
  status: string;
  unitEconomics: BmUnitEconomics | null;
}

interface BmExperimentData {
  id: string;
  hypothesisId: string;
  title: string;
...
interface BmRevenueObjectiveData {
  id: string;
  period: string;
...
// API가 아직 비어있거나 응답 실패 시 보여줄 초기 시드값(v1.2 §4, §6 문서 예시 그대로) — DB에 실제 데이터가 쌓이면 대체된다.
const SEED_HYPOTHESES: BmHypothesisData[] = [
  {
    id: 'seed-job-posting', name: '급구 현장 공고', customerSegment: '현장 운영사·협력사',
    problem: '급한 인력을 빠르게 확보하기 어렵다.', valueProposition: '검증된 지역 기술자에게 공고를 빠르게 노출',
    pricingHypothesis: '건당 3만~10만원', revenueType: '거래형',
    currentEvidence: '전체 문의 24건 중 8곳 결제 의향 확인 (전체 문의 기준 33%, 2026.07 1주간)', evidenceLevel: 'PILOT',
    nextExperiment: '유료 공고 5건 판매', successCriteria: '결제 전환율 20% 이상', failureCriteria: '30개 제안 후 유료 고객 2개 미만',
    owner: '대표', dueDate: null, status: 'P0_NOW', unitEconomics: null,
  },
  {
    id: 'seed-workspace', name: 'Partner Workspace', customerSegment: '협력사·현장 운영사',
    problem: '출역·정산·평가 데이터가 분산돼 있다.', valueProposition: '출역·팀·정산 증빙, 평가·재요청 통합 관리',
    pricingHypothesis: '월 구독 + 초기 설정비 (월 4만~15만원)', revenueType: '반복매출',
    currentEvidence: '4곳 인터뷰 · 기능 PoC 요구 확인 (2026.07 B2B)', evidenceLevel: 'INTERVIEW',
    nextExperiment: '2개사 유료 PoC', successCriteria: '유료전환 1건 이상', failureCriteria: null,
    owner: '대표', dueDate: null, status: 'P1_NEXT', unitEconomics: null,
  },
  {
    id: 'seed-profile-access', name: '검증 프로필·팀 데이터', customerSegment: '현장 운영사·협력사',
    problem: '기술자의 실제 경험과 자격을 확인하기 어렵다.',
    valueProposition: '경력·자격·안전교육·현장 이력을 구조화해 신뢰 판단 지원(급구 공고·Workspace 매칭 품질을 높이는 신뢰 데이터 계층)',
    pricingHypothesis: '상세 검증정보 열람·후보 저장·연락 요청·대량 후보 조회·팀 단위 검증 과금', revenueType: '부가과금',
    currentEvidence: '12건 상담 중 5곳 즉시 도입 의향 (2026.07 B2B)', evidenceLevel: 'SURVEY',
    nextExperiment: '유료 열람 옵션 PoC', successCriteria: '유료 열람 전환 3건 이상', failureCriteria: null,
    owner: '대표', dueDate: null, status: 'P1_NEXT', unitEconomics: null,
  },
  {
    id: 'seed-field-pass', name: 'MONO Field Pass', customerSegment: '대형 현장·원청·협력사 관리조직',
    problem: '신원·자격·교육·출입·근태가 분리돼 있다.', valueProposition: '현장 투입에 필요한 신원, 자격, 교육, 출입권한과 출역 기록을 연결',
    pricingHypothesis: '현장 구독 + 구축비 + 인증 건수 과금', revenueType: 'Enterprise 구독',
    currentEvidence: '아키텍처 설계 · 센스톤 기술 미팅 예정', evidenceLevel: 'TECH_REVIEW',
    nextExperiment: 'OTAC 기술 적합성 확인 · 현장 PoC 후보 확보',
    successCriteria: '4~8주 PoC 범위 합의 / 고객 예산·구매부서 확인 / 기술협력 또는 PoC 문서 확보', failureCriteria: null,
    owner: '대표', dueDate: null, status: 'P0_NOW', unitEconomics: null,
  },
];

const SEED_REVENUE_OBJECTIVES: BmRevenueObjectiveData[] = [
  {
    id: 'seed-objective-1', period: '2026 Q3', targetRevenue: '유료 기업 고객 5개사',
    keyBm: '급구 현장 공고 · Partner Workspace · Field Pass', successCriteria: '재구매 2개사 / Workspace 유료 PoC 1건 / Field Pass PoC 범위 합의 1건',
    owner: '대표', status: 'ACTIVE',
  },
];
...
// Component
// ─────────────────────────────────────────────
export default function BmBoard({ role }: { role: 'admin' | 'mentor' }) {
  ...
  // v1.2 — Active BM Hypotheses / Experiment Board / Decision Log / Next Actions / Revenue Objective
  const [hypotheses, setHypotheses] = useState<BmHypothesisData[]>(SEED_HYPOTHESES);
  const [experiments, setExperiments] = useState<BmExperimentData[]>([]);
  ...
  const reloadDecisions = () => fetch('/api/bm/decisions', { cache: 'no-store' }).then((r) => r.json()).then((d) => { if (Array.isArray(d)) setDecisions(d); }).catch(() => undefined);
  const reloadNextActions = () => fetch('/api/bm/next-actions', { cache: 'no-store' }).then((r) => r.json()).then((d) => { if (Array.isArray(d)) setNextActions(d); }).catch(() => undefined);
  const reloadRevenueObjectives = () => fetch('/api/bm/revenue-objectives', { cache: 'no-store' }).then((r) => r.json()).then((d) => { if (Array.isArray(d) && d.length > 0) setRevenueObjectives(d); }).catch(() => undefined);
  ...
  const updateHypothesisStatus = (id: string, status: string) => {
    if (isMentor) return;
    fetch(`/api/bm/hypotheses/${id}`, { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ status }) })
      .then(reloadHypotheses);
  };

  const [showAddExperiment, setShowAddExperiment] = useState(false);
  const [newExperiment, setNewExperiment] = useState({ hypothesisId: '', title: '', method: '', successCriteria: '' });
  const submitNewExperiment = () => {
    if (!newExperiment.hypothesisId || !newExperiment.title) return;
    fetch('/api/bm/experiments', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ ...newExperiment, stage: 'BACKLOG' }) })
      .then(() => { track('bm_experiment_created', { hypothesisId: newExperiment.hypothesisId }); setNewExperiment({ hypothesisId: '', title: '', method: '', successCriteria: '' }); setShowAddExperiment(false); reloadExperiments(); });
  };

  const [showAddDecision, setShowAddDecision] = useState(false);
  const [newDecision, setNewDecision] = useState({ hypothesisId: '', bmName: '', decision: 'KEEP', rationale: '', approver: '' });
  const submitNewDecision = () => {
    if (!newDecision.bmName || !newDecision.rationale || !newDecision.approver) return;
    fetch('/api/bm/decisions', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(newDecision) })
      .then(() => { track('bm_decision_recorded', { decision: newDecision.decision }); setNewDecision({ hypothesisId: '', bmName: '', decision: 'KEEP', rationale: '', approver: '' }); setShowAddDecision(false); reloadDecisions(); });
  };

  const [showAddAction, setShowAddAction] = useState(false);
  const [newAction, setNewAction] = useState({ action: '', owner: '', hypothesisId: '', successCriteria: '' });
  const submitNewAction = () => {
    if (!newAction.action || !newAction.owner) return;
    fetch('/api/bm/next-actions', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(newAction) })
      .then(() => { setNewAction({ action: '', owner: '', hypothesisId: '', successCriteria: '' }); setShowAddAction(false); reloadNextActions(); });
  };

  const toggleActionStatus = (a: BmNextActionData) => {
    if (isMentor) return;
    const next = a.status === 'DONE' ? 'OPEN' : 'DONE';
    fetch(`/api/bm/next-actions/${a.id}`, { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ status: next }) })
  ...
  const recommendationsList = [
    {
      ...
      pilotStats: {
        ...
        inquiries: '24건 문의',
        willingness: '8곳 결제의향 확인 (전체 문의 기준 33%)',
        color: '#10b981',
        bg: '#f0fdf4'
      }
    },
      ...
      <style>{`
        * { box-sizing: border-box; }
        ...
        .info-modal-panel {
          ...
          animation: modalPop 0.18s cubic-bezier(0.32, 0.72, 0, 1);
        }
        ...
        /* v1.1 신규 섹션 그리드 */
        .core-bm-grid { display: grid; gap: 18px; grid-template-columns: repeat(3, 1fr); }
        .team-readiness-grid { display: grid; gap: 12px; grid-template-columns: repeat(2, 1fr); }
        .regulatory-grid { display: grid; gap: 10px; grid-template-columns: 1fr; }
        @media (max-width: 1023px) {
          .core-bm-grid { grid-template-columns: 1fr; }
          .team-readiness-grid { grid-template-columns: 1fr; }
        }
        @keyframes slideInRight { from { transform: translateX(40px); opacity: 0; } to { transform: translateX(0); opacity: 1; } }
        .animate-slide { animation: slideInRight 0.25s ease; }
      `}</style>

      {/* ── Header (내부 BM 검증 도구 — 서비스 소개/영업 카피는 /service, /partner로 이동) ── */}
      <header
        style={{
        ...
        <div style={{ maxWidth: 1400, margin: '0 auto' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <h1 style={{ fontSize: 20, fontWeight: 950, margin: 0, letterSpacing: '-0.02em', color: '#0f172a' }}>
                MONO BM 검증 보드
              </h1>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button
                onClick={() => { clearCompetitors(); clearSegments(); clearFeatures(); setSelectedScenario(null); }}
                style={{ padding: '10px 20px', background: '#ffffff', border: '1px solid #cbd5e1', borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: 'pointer', color: '#475569' }}
              >
                필터 초기화
              </button>
              <button
          ...
          <p style={{ fontSize: 13, color: '#64748b', margin: '6px 0 0 0', fontWeight: 650, lineHeight: 1.5 }}>
            BM 가설·가격가설·검증 실험·규제 게이트·다음 액션을 관리하는 내부 도구입니다. 서비스 소개는 /service, 기업 영업은 /partner를 참고하세요.
          </p>
        </div>
      </header>
      ...
      <main style={{ maxWidth: 1400, margin: '0 auto', padding: '28px 28px 60px' }}>
        ...
        <section style={{ marginBottom: 32 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 12 }}>
            {revenueObjectives.map((o) => (
              <div key={o.id} style={{ background: '#ffffff', border: '2px solid #cbd5e1', borderRadius: 16, padding: '18px 20px', display: 'flex', flexDirection: 'column', gap: 8 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: 15, fontWeight: 950, color: '#0f172a' }}>{o.period}</span>
                  <span style={{ fontSize: 10.5, fontWeight: 900, color: '#4f46e5', background: 'rgba(79,70,229,0.08)', padding: '3px 9px', borderRadius: 6 }}>{o.status}</span>
                </div>
                <div style={{ fontSize: 13, color: '#334155', fontWeight: 700 }}>🎯 목표: {o.targetRevenue}</div>
                <div style={{ fontSize: 12.5, color: '#64748b', fontWeight: 650 }}>핵심 BM: {o.keyBm}</div>
                <div style={{ fontSize: 12.5, color: '#64748b', fontWeight: 650 }}>성공 기준: {o.successCriteria}</div>
                <div style={{ fontSize: 11.5, color: '#94a3b8', fontWeight: 650, marginTop: 'auto' }}>책임자: {o.owner}</div>
              </div>
            ))}
          </div>
        </section>
        ...
        <section id="core-bm" style={{ marginBottom: 32, scrollMarginTop: 70 }}>
          <div style={{ fontSize: 13, fontWeight: 850, color: '#4f46e5', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 12 }}>
            Active BM Hypotheses — 어떤 고객이 어떤 문제를 위해 실제 돈을 내는가
          </div>
          <div className="core-bm-grid">
            {hypotheses.map((h) => (
              <div
                key={h.id}
                onClick={() => track('bm_hypothesis_opened', { hypothesisId: h.id, name: h.name })}
                style={{ border: '2px solid #e2e8f0', background: '#ffffff', borderRadius: 16, padding: 20, display: 'flex', flexDirection: 'column', gap: 8 }}
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                  <span style={{ fontSize: 18, fontWeight: 950, color: '#0f172a' }}>{h.name}</span>
                  {isMentor ? (
                    <Badge badge={HYPOTHESIS_STATUS_BADGE[h.status] ?? HYPOTHESIS_STATUS_BADGE.P1_NEXT} />
                  ) : (
                    <select
                      value={h.status}
                      onClick={(e) => e.stopPropagation()}
                      onChange={(e) => updateHypothesisStatus(h.id, e.target.value)}
                      style={{ fontSize: 11, fontWeight: 800, border: '1px solid #e2e8f0', borderRadius: 6, padding: '3px 6px', color: '#334155' }}
                    >
                      {Object.keys(HYPOTHESIS_STATUS_BADGE).map((s) => <option key={s} value={s}>{HYPOTHESIS_STATUS_BADGE[s].label}</option>)}
                    </select>
                  )}
                </div>
                <div style={{ fontSize: 12, color: '#64748b', fontWeight: 700 }}>고객: <strong style={{ color: '#334155' }}>{h.customerSegment}</strong></div>
                <p style={{ margin: 0, fontSize: 12.5, color: '#475569', fontWeight: 650, lineHeight: 1.55, wordBreak: 'keep-all' }}>{h.problem}</p>
                <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 10, padding: '10px 12px', fontSize: 12, display: 'flex', flexDirection: 'column', gap: 5 }}>
                  <div>💰 가격 가설: <strong>{isMentor ? '🔒 권한 필요' : h.pricingHypothesis}</strong></div>
                  <div>📈 수익 유형: <strong>{h.revenueType}</strong></div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                    <Badge badge={evidenceBadgeFor(h.evidenceLevel)} />
                    <span>{h.currentEvidence}</span>
                  </div>
                  <div>🧪 다음 실험: <strong>{h.nextExperiment}</strong></div>
                  <div>✅ 성공 기준: <strong>{h.successCriteria}</strong></div>
        ...
        <section style={{ background: '#ffffff', border: '2px solid #cbd5e1', borderRadius: 16, padding: 28, marginBottom: 32, boxShadow: '0 4px 20px rgba(0,0,0,0.02)' }}>
          ...
          <p style={{ fontSize: 13, color: '#64748b', fontWeight: 650, margin: '0 0 18px 0', lineHeight: 1.5 }}>
            데이터가 없는 지표는 0이 아니라 &quot;측정 준비 중&quot;으로 표시합니다. 모든 수치는 측정기간·증거 배지와 함께 제공됩니다.
          </p>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(230px, 1fr))', gap: 12 }}>
            {[
              { cat: '공급', label: '검증 완료 기술자 프로필 수', value: '측정 준비 중', evidence: EVIDENCE_BADGE.hypothesis },
              { cat: '매출', label: '유료 기능 관심', value: liveMetrics ? `${liveMetrics['유료 기능 관심']}건` : '측정 준비 중', evidence: EVIDENCE_BADGE.survey, period: '누적' },
              { cat: '반복성', label: '재구매 기업 수·재구매율', value: '측정 준비 중', evidence: EVIDENCE_BADGE.hypothesis },
              { cat: '매칭', label: '공고 등록 후 첫 적격 후보 확보 시간', value: '측정 준비 중', evidence: EVIDENCE_BADGE.hypothesis },
              { cat: '품질', label: '지원자 대비 실제 출역률', value: '측정 준비 중', evidence: EVIDENCE_BADGE.hypothesis },
              { cat: 'SaaS', label: 'Workspace 활성 기업·MRR', value: '측정 준비 중', evidence: EVIDENCE_BADGE.interview, period: '2026.07' },
              { cat: '계약', label: 'PoC 협의 관심', value: liveMetrics ? `${liveMetrics['PoC 협의 관심']}건` : '측정 준비 중', evidence: EVIDENCE_BADGE.survey, period: '누적' },
              { cat: '계약', label: '팀 등록 의향', value: liveMetrics ? `${liveMetrics['팀 등록 의향']}건` : '측정 준비 중', evidence: EVIDENCE_BADGE.survey, period: '누적' },
              { cat: 'Field Pass', label: '현장 수·사용자 수·인증 건수', value: '기술 검토 중', evidence: EVIDENCE_BADGE.tech_review },
              { cat: '보안', label: '인증 성공률·차단률·응답시간', value: '기술 검토 중', evidence: EVIDENCE_BADGE.tech_review },
              { cat: 'Enterprise', label: '대기업 PoC·연동 협의 수', value: '검증 예정', evidence: EVIDENCE_BADGE.hypothesis },
            ].map((k, idx) => (
              <div key={idx} style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 12, padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 6 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: 10.5, fontWeight: 900, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{k.cat}</span>
                  <Badge badge={k.evidence} />
                </div>
                <div style={{ fontSize: 12.5, fontWeight: 700, color: '#475569', lineHeight: 1.4 }}>{k.label}</div>
                <div style={{ fontSize: 17, fontWeight: 950, color: '#0f172a' }}>{k.value}</div>
                {k.period && <div style={{ fontSize: 10.5, color: '#94a3b8', fontWeight: 650 }}>측정기간: {k.period}</div>}
              </div>
            ))}
          </div>
        </section>

        {/* ── Experiment Board (v1.2 §9) ── */}
        <section style={{ marginBottom: 32 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, flexWrap: 'wrap', gap: 8 }}>
            <div style={{ fontSize: 13, fontWeight: 850, color: '#4f46e5', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
              Experiment Board
            </div>
            {!isMentor && (
              <button onClick={() => setShowAddExperiment((v) => !v)} style={{ fontSize: 12, fontWeight: 800, color: '#4f46e5', background: '#eef2ff', border: '1px solid #c7d2fe', borderRadius: 8, padding: '6px 12px', cursor: 'pointer' }}>
                + 실험 추가
              </button>
            )}
          </div>
          {showAddExperiment && !isMentor && (
            <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 12, padding: 14, marginBottom: 14, display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              <select value={newExperiment.hypothesisId} onChange={(e) => setNewExperiment({ ...newExperiment, hypothesisId: e.target.value })} style={{ padding: '7px 10px', border: '1px solid #cbd5e1', borderRadius: 8, fontSize: 12.5 }}>
                <option value="">연결 BM 선택</option>
                {hypotheses.map((h) => <option key={h.id} value={h.id}>{h.name}</option>)}
              </select>
              <input placeholder="가설/실험 제목" value={newExperiment.title} onChange={(e) => setNewExperiment({ ...newExperiment, title: e.target.value })} style={{ flex: 1, minWidth: 160, padding: '7px 10px', border: '1px solid #cbd5e1', borderRadius: 8, fontSize: 12.5 }} />
              <input placeholder="실험 방법" value={newExperiment.method} onChange={(e) => setNewExperiment({ ...newExperiment, method: e.target.value })} style={{ flex: 1, minWidth: 140, padding: '7px 10px', border: '1px solid #cbd5e1', borderRadius: 8, fontSize: 12.5 }} />
              <input placeholder="성공 기준" value={newExperiment.successCriteria} onChange={(e) => setNewExperiment({ ...newExperiment, successCriteria: e.target.value })} style={{ flex: 1, minWidth: 140, padding: '7px 10px', border: '1px solid #cbd5e1', borderRadius: 8, fontSize: 12.5 }} />
        ...
        {/* ── Compliance Gates (v1.2 §11 / 구 Regulatory Readiness §24,§26) ── */}
        <section style={{ background: '#ffffff', border: '2px solid #cbd5e1', borderRadius: 16, padding: 28, marginBottom: 32, boxShadow: '0 4px 20px rgba(0,0,0,0.02)' }} onMouseEnter={() => track('bm_compliance_gate_opened', {})}>
          <div style={{ fontSize: 13, fontWeight: 850, color: '#4f46e5', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 4 }}>
            Compliance Gates
          </div>
          <p style={{ fontSize: 13, color: '#64748b', fontWeight: 650, margin: '0 0 18px 0', lineHeight: 1.55, wordBreak: 'keep-all' }}>
            본 페이지는 제품·개발 요구사항이며 법률의견서가 아닙니다. 신고·등록 및 규제 판단은 실제 서비스 흐름과 계약구조를 기준으로 별도 확인합니다.
          </p>

          {/* 대표 예시 3건 (§26) */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(230px, 1fr))', gap: 10, marginBottom: 20 }}>
            {[
              { name: 'MONO Field Pass 위치 체크인', badge: COMPLIANCE_BADGE.report_required, action: '개인위치기반서비스 신고·위치약관', fallback: '신고 전 QR·현장코드 방식', target: '2027.Q1' },
              { name: 'OTAC 일회성 인증', badge: COMPLIANCE_BADGE.partner_required, action: '센스톤 기술·라이선스 협의', fallback: '합의 전 TECH REVIEW 표시', target: 'PoC 협의 후' },
              { name: '기술자 직접 파견', badge: COMPLIANCE_BADGE.blocked, action: '—', fallback: 'MONO가 고용·지휘·급여 지급하지 않음', target: '—' },
            ].map((ex) => (
              <div key={ex.name} style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 12, padding: '14px 16px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, gap: 8 }}>
                  <span style={{ fontSize: 13, fontWeight: 900, color: '#0f172a' }}>{ex.name}</span>
                  <Badge badge={ex.badge} />
                </div>
                <div style={{ fontSize: 11.5, color: '#64748b', fontWeight: 650, lineHeight: 1.5 }}>
                  필요 조치: <strong style={{ color: '#334155' }}>{ex.action}</strong><br />
                  대응: <strong style={{ color: '#334155' }}>{ex.fallback}</strong><br />
                  목표: <strong style={{ color: '#334155' }}>{ex.target}</strong>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="regulatory-grid">
            {[
              ['급구 공고', '직업정보제공사업 신고 검토', '신고 완료 후 공개', COMPLIANCE_BADGE.report_required],
              ['프로필 검색', '정보제공 범위 유지', 'P0 가능', COMPLIANCE_BADGE.ready],
              ['후보 추천·성공 수수료', '유료직업소개사업 등록', '등록 완료 후', COMPLIANCE_BADGE.license_gate],
              ['기술자 일괄 투입', '파견·근로자공급 위험', 'BLOCKED', COMPLIANCE_BADGE.blocked],
              ['위치 기반 공고', '개인위치기반서비스 신고', '신고 후 GPS 활성화', COMPLIANCE_BADGE.report_required],
              ['GPS 출역 체크인', '위치약관·동의·보호조치', '신고 후', COMPLIANCE_BADGE.report_required],
              ['출역·정산 SaaS', '계산·리포트 범위', 'P0/P1', COMPLIANCE_BADGE.ready],
              ['임금·노무 신고 대행', '세무사·노무사 제휴', '제휴 후', COMPLIANCE_BADGE.partner_required],
              ['결제·정산', '등록 PG 연동', 'PG 후', COMPLIANCE_BADGE.partner_required],
              ['AI 안전 가이드', '법정교육 대체 불가 표시', '즉시 가능', COMPLIANCE_BADGE.ready],
              ['법정 안전교육', '등록 교육기관 연계', '제휴 후', COMPLIANCE_BADGE.partner_required],
              ['외국인 매칭', '체류자격별 제한', '제한·정부절차 연결', COMPLIANCE_BADGE.legal_review],
              ['금융·보험', '등록 사업자 연결', '제휴 후', COMPLIANCE_BADGE.partner_required],
              ['Field Pass 위치 인증', '위치정보 동의·신고', '신고 후', COMPLIANCE_BADGE.report_required],
              ['출입·근태 기록', '개인정보 목적·보유기간·권한', '정책 완료 후', COMPLIANCE_BADGE.legal_review],
              ['OTAC', '센스톤 라이선스·IP·책임 범위', '계약 후', COMPLIANCE_BADGE.partner_required],
              ['기업 출입 API', '고객 시스템 책임범위·보안심사', '계약·심사 후', COMPLIANCE_BADGE.legal_review],
              ['생체인증', '별도 민감정보 검토', '기본 비활성화', COMPLIANCE_BADGE.blocked],
            ].map(([name, action, gate, badge]) => (
              <div key={name as string} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10, padding: '9px 14px', background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 8, flexWrap: 'wrap' }}>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px 12px', alignItems: 'baseline', flex: 1, minWidth: 220 }}>
                  <span style={{ fontSize: 13, fontWeight: 900, color: '#0f172a' }}>{name as string}</span>
                  <span style={{ fontSize: 11.5, color: '#64748b', fontWeight: 600 }}>{action as string} → {gate as string}</span>
                </div>
                <Badge badge={badge as BadgeStyle} />
              </div>
            ))}
          </div>
        </section>

        {/* ── Decision Log (v1.2 §12) ── */}
        <section style={{ background: '#ffffff', border: '2px solid #cbd5e1', borderRadius: 16, padding: 28, marginBottom: 32, boxShadow: '0 4px 20px rgba(0,0,0,0.02)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, flexWrap: 'wrap', gap: 8 }}>
            <div style={{ fontSize: 13, fontWeight: 850, color: '#4f46e5', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
              Decision Log
            ...
            {!isMentor && (
              <button onClick={() => setShowAddDecision((v) => !v)} style={{ fontSize: 12, fontWeight: 800, color: '#4f46e5', background: '#eef2ff', border: '1px solid #c7d2fe', borderRadius: 8, padding: '6px 12px', cursor: 'pointer' }}>
                + 결정 기록
              </button>
            )}
          </div>
          {showAddDecision && !isMentor && (
            <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 12, padding: 14, marginBottom: 14, display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              <select value={newDecision.hypothesisId} onChange={(e) => { const h = hypotheses.find((x) => x.id === e.target.value); setNewDecision({ ...newDecision, hypothesisId: e.target.value, bmName: h?.name ?? newDecision.bmName }); }} style={{ padding: '7px 10px', border: '1px solid #cbd5e1', borderRadius: 8, fontSize: 12.5 }}>
                <option value="">연결 BM 선택</option>
                {hypotheses.map((h) => <option key={h.id} value={h.id}>{h.name}</option>)}
              </select>
              <select value={newDecision.decision} onChange={(e) => setNewDecision({ ...newDecision, decision: e.target.value })} style={{ padding: '7px 10px', border: '1px solid #cbd5e1', borderRadius: 8, fontSize: 12.5 }}>
                {Object.keys(DECISION_BADGE).map((d) => <option key={d} value={d}>{d}</option>)}
              </select>
              <input placeholder="근거" value={newDecision.rationale} onChange={(e) => setNewDecision({ ...newDecision, rationale: e.target.value })} style={{ flex: 1, minWidth: 160, padding: '7px 10px', border: '1px solid #cbd5e1', borderRadius: 8, fontSize: 12.5 }} />
              <input placeholder="승인자" value={newDecision.approver} onChange={(e) => setNewDecision({ ...newDecision, approver: e.target.value })} style={{ width: 100, padding: '7px 10px', border: '1px solid #cbd5e1', borderRadius: 8, fontSize: 12.5 }} />
              <button onClick={submitNewDecision} style={{ padding: '7px 16px', background: '#4f46e5', color: '#fff', border: 'none', borderRadius: 8, fontSize: 12.5, fontWeight: 800, cursor: 'pointer' }}>등록</button>
            </div>
          )}
          {decisions.length === 0 ? (
            <div style={{ fontSize: 13, color: '#94a3b8', fontWeight: 650 }}>아직 기록된 결정이 없습니다.</div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {decisions.map((d) => (
                <div key={d.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10, padding: '10px 14px', background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 8, flexWrap: 'wrap' }}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 2, flex: 1, minWidth: 220 }}>
                    <div style={{ fontSize: 13, fontWeight: 900, color: '#0f172a' }}>{d.bmName}</div>
                    <div style={{ fontSize: 11.5, color: '#64748b', fontWeight: 600 }}>{d.rationale} · 승인자: {d.approver}</div>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ fontSize: 11, color: '#94a3b8' }}>{d.createdAt.slice(0, 10)}</span>
                    <Badge badge={DECISION_BADGE[d.decision] ?? DECISION_BADGE.KEEP} />
                  </div>
                </div>
              ))}
            </div>
        ...
        <section style={{ background: '#ffffff', border: '2px solid #cbd5e1', borderRadius: 16, padding: 28, marginBottom: 32, boxShadow: '0 4px 20px rgba(0,0,0,0.02)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, flexWrap: 'wrap', gap: 8 }}>
            ...
            {!isMentor && (
              <button onClick={() => setShowAddAction((v) => !v)} style={{ fontSize: 12, fontWeight: 800, color: '#4f46e5', background: '#eef2ff', border: '1px solid #c7d2fe', borderRadius: 8, padding: '6px 12px', cursor: 'pointer' }}>
                + 액션 추가
              </button>
            )}
          </div>
          {showAddAction && !isMentor && (
            <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 12, padding: 14, marginBottom: 14, display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              <input placeholder="액션" value={newAction.action} onChange={(e) => setNewAction({ ...newAction, action: e.target.value })} style={{ flex: 1, minWidth: 160, padding: '7px 10px', border: '1px solid #cbd5e1', borderRadius: 8, fontSize: 12.5 }} />
              <input placeholder="담당자" value={newAction.owner} onChange={(e) => setNewAction({ ...newAction, owner: e.target.value })} style={{ width: 100, padding: '7px 10px', border: '1px solid #cbd5e1', borderRadius: 8, fontSize: 12.5 }} />
              <select value={newAction.hypothesisId} onChange={(e) => setNewAction({ ...newAction, hypothesisId: e.target.value })} style={{ padding: '7px 10px', border: '1px solid #cbd5e1', borderRadius: 8, fontSize: 12.5 }}>
                <option value="">연결 BM(선택)</option>
                {hypotheses.map((h) => <option key={h.id} value={h.id}>{h.name}</option>)}
              </select>
              <input placeholder="성공 기준" value={newAction.successCriteria} onChange={(e) => setNewAction({ ...newAction, successCriteria: e.target.value })} style={{ flex: 1, minWidth: 140, padding: '7px 10px', border: '1px solid #cbd5e1', borderRadius: 8, fontSize: 12.5 }} />
              <button onClick={submitNewAction} style={{ padding: '7px 16px', background: '#4f46e5', color: '#fff', border: 'none', borderRadius: 8, fontSize: 12.5, fontWeight: 800, cursor: 'pointer' }}>등록</button>
            </div>
          )}
          {nextActions.length === 0 ? (
            <div style={{ fontSize: 13, color: '#94a3b8', fontWeight: 650 }}>등록된 다음 액션이 없습니다.</div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {nextActions.map((a) => (
                <div key={a.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10, padding: '10px 14px', background: a.status === 'DONE' ? '#f0fdf4' : '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 8, flexWrap: 'wrap' }}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 2, flex: 1, minWidth: 220 }}>
                    <div style={{ fontSize: 13, fontWeight: 900, color: '#0f172a', textDecoration: a.status === 'DONE' ? 'line-through' : 'none' }}>{a.action}</div>
                    <div style={{ fontSize: 11.5, color: '#64748b', fontWeight: 600 }}>
                      담당자: {a.owner}{a.dueDate ? ` · 마감 ${a.dueDate.slice(0, 10)}` : ''}{a.successCriteria ? ` · 성공기준: ${a.successCriteria}` : ''}{a.blocker ? ` · 차단요인: ${a.blocker}` : ''}
                    </div>
                  </div>
                  {!isMentor && (
                    <button onClick={() => toggleActionStatus(a)} style={{ fontSize: 11.5, fontWeight: 800, color: a.status === 'DONE' ? '#166534' : '#4f46e5', background: 'transparent', border: '1px solid #e2e8f0', borderRadius: 6, padding: '4px 10px', cursor: 'pointer' }}>
                      {a.status === 'DONE' ? '완료됨' : '완료 처리'}
                    </button>
                  )}
                </div>
              ))}
        ...
        {/* ── Expansion Roadmap (v1.1 §15 — 기본 접힘 아코디언) ── */}
        <section style={{ marginBottom: 36 }}>
          <details onToggle={(e) => { if ((e.target as HTMLDetailsElement).open) track('bm_expansion_roadmap_opened', {}); }}>
            <summary style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, cursor: 'pointer', listStyle: 'none' }}>
              <div style={{ fontSize: 15, fontWeight: 900, color: '#475569', display: 'flex', alignItems: 'center', gap: 6 }}>
                <span>📈</span> Expansion Roadmap — P1 운영 확장 · P2 Enterprise·Data 확장 (펼쳐보기)
              </div>
              <span style={{ fontSize: 12, fontWeight: 800, background: '#f1f5f9', color: '#475569', padding: '3px 10px', borderRadius: 6 }}>중장기 과제</span>
            </summary>
          <div className="recommendation-grid" style={{ gap: '16px' }}>
            {recommendationsList.filter(r => r.priority !== 'P0' || r.id === 'ai-guide-pro').map((card) => {
              const ROADMAP_RELABEL: Record<string, string> = { 'ai-guide-pro': 'Growth Engine · 부가수익' };
              const displayLabel = ROADMAP_RELABEL[card.id] ?? card.label;
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '28px', alignItems: 'stretch' }}>
            ...
            <div style={{ background: '#f8fafc', borderRadius: '12px', padding: '20px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
              ...
              <ul style={{ margin: 0, paddingLeft: '20px', fontSize: '13px', color: '#475569', lineHeight: '1.6', fontWeight: '600' }}>
                ...
                <li style={{ marginBottom: '8px' }}>
                  <strong>좌상단(2사분면)</strong>은 타사가 이미 입증한 모델로, 1주일간의 파일럿을 통해 전체 문의 24건 중 8곳의 결제 의향(전체 문의 기준 33%)을 실측 확보 완료했습니다.
                </li>
                <li>
                  영업 주기가 긴 SaaS 모델(<span style={{ color: '#059669' }}>Workspace 구독</span>)은 우선 검증 대상(P0)에서 제외하고 중기 P1 로드맵으로 재조정하여 현실성을 높였습니다.
                </li>
              </ul>
            </div>
          </div>
