...
// ─────────────────────────────────────────────
type EvidenceType = 'paid' | 'pilot' | 'loi' | 'interview' | 'survey' | 'hypothesis' | 'tech_review' | 'poc_planned' | 'partnered';
    ...
    >
      ...
      >
        <div style={{ maxWidth: 1400, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
          <span style={{ fontSize: 15, fontWeight: 950, color: '#0f172a', letterSpacing: '-0.02em' }}>MONO</span>
          <button
      ...
      {/* ── Hero (BM 개발 개선 요청서 v1.1 §4 Section 1) ── */}
      <section
        style={{
          background: 'linear-gradient(180deg, #0b1224 0%, #101a35 60%, #0f172a 100%)',
          padding: '56px 20px 44px',
        }}
      >
        <div style={{ maxWidth: 1000, margin: '0 auto', textAlign: 'center' }}>
          <h1 style={{ fontSize: 'clamp(26px, 4vw, 40px)', fontWeight: 950, margin: 0, letterSpacing: '-0.03em', color: '#ffffff', lineHeight: 1.28 }}>
            현장 기술자 채용에서 출입·정산까지,<br />하나의 신뢰 인프라
          </h1>
          <p style={{ fontSize: 15.5, color: 'rgba(226,232,240,0.88)', fontWeight: 650, lineHeight: 1.65, margin: '18px auto 0', maxWidth: 760, wordBreak: 'keep-all' }}>
            MONO는 급구 현장 공고로 첫 거래를 만들고, 검증된 기술자·팀 데이터와 Partner Workspace로 반복매출을 만듭니다. 이후 MONO Field Pass를 통해 신원·자격·교육·출입권한·근태 데이터를 연결해 대기업 협력사 관리와 컴플라이언스 영역으로 확장합니다.
          </p>
          <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: 8, marginTop: 22 }}>
            {['거래형 매출', '구독형 ARR', '검증된 기술자 데이터', '산업현장 Trust Infrastructure', 'Enterprise API'].map((tag) => (
              <span key={tag} style={{ fontSize: 12.5, fontWeight: 800, color: '#c7d2fe', background: 'rgba(129,140,248,0.12)', border: '1px solid rgba(129,140,248,0.35)', padding: '5px 13px', borderRadius: 999 }}>
                {tag}
              </span>
            ))}
          </div>
          <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: 10, marginTop: 28 }}>
            <button
              onClick={() => { track('bm_poc_cta_clicked', { source: 'hero' }); window.location.href = '#poc-cta'; }}
              style={{ padding: '13px 24px', background: '#ffffff', border: 'none', borderRadius: 12, fontSize: 14.5, fontWeight: 900, cursor: 'pointer', color: '#111827' }}
            >
              PoC 문의하기
            </button>
            <button
              onClick={() => { window.location.href = '#core-bm'; }}
              style={{ padding: '13px 24px', background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.25)', borderRadius: 12, fontSize: 14.5, fontWeight: 800, cursor: 'pointer', color: '#ffffff' }}
            >
              핵심 BM 보기
            </button>
            <button
              onClick={() => { track('bm_field_pass_cta_clicked', { source: 'hero' }); window.location.href = '#field-pass'; }}
              style={{ padding: '13px 24px', background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.25)', borderRadius: 12, fontSize: 14.5, fontWeight: 800, cursor: 'pointer', color: '#ffffff' }}
            >
              Field Pass 보기
            </button>
      ...
      <main style={{ maxWidth: 1400, margin: '0 auto', padding: '28px 28px 60px' }}>
        ...
        {/* ── Revenue Flow (v1.1 §4 Section 2 — 5단계) ── */}
        <section style={{ marginBottom: 32 }} onMouseEnter={() => track('bm_revenue_flow_scrolled', {})}>
          <div style={{ fontSize: 13, fontWeight: 850, color: '#4f46e5', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 12 }}>
            Revenue Flow — 첫 거래에서 산업현장 신뢰 인프라까지
          </div>
          <div className="recommendation-grid" style={{ gap: 12 }}>
            {[
              { step: '1', title: '급구 현장 공고', role: 'Market Entry', value: '현장 운영사·협력사의 긴급 인력 수요 해결', revenue: '건별·패키지 과금', status: PRODUCT_STATUS_BADGE.pilot },
              { step: '2', title: '검증 프로필·팀 데이터', role: 'Trust Layer', value: '경력·자격·안전교육·현장 이력 확인, 매칭 신뢰도 향상', revenue: '후보 저장·연락 옵션', status: PRODUCT_STATUS_BADGE.validating },
              { step: '3', title: 'Partner Workspace', role: 'Operations SaaS', value: '출역·팀·정산 증빙, 평가·재요청 통합 관리', revenue: '월 구독·도입비', status: PRODUCT_STATUS_BADGE.pilot },
              { step: '4', title: 'MONO Field Pass', role: 'Enterprise Trust', value: '신원·자격·교육·출입권한을 통합, 대기업 협력사 관리', revenue: '현장 구독·API·라이선스', status: PRODUCT_STATUS_BADGE.prototype },
              { step: '5', title: 'Data & Compliance', role: 'Enterprise & Data', value: '협력사·안전·감사 데이터, 컴플라이언스 리포트', revenue: 'Enterprise 계약', status: PRODUCT_STATUS_BADGE.roadmap },
            ].map((s) => (
              <div key={s.step} style={{ background: '#ffffff', border: '1.5px solid #e2e8f0', borderRadius: 14, padding: '16px 14px', display: 'flex', flexDirection: 'column', gap: 8 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: 11, fontWeight: 950, color: '#ffffff', background: '#4f46e5', width: 20, height: 20, borderRadius: 999, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{s.step}</span>
                  <Badge badge={s.status} />
                </div>
                <div style={{ fontSize: 11.5, fontWeight: 800, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.04em' }}>{s.role}</div>
                <div style={{ fontSize: 15, fontWeight: 950, color: '#0f172a' }}>{s.title}</div>
                <div style={{ fontSize: 12.5, color: '#475569', fontWeight: 650, lineHeight: 1.5, wordBreak: 'keep-all' }}>{s.value}</div>
                <div style={{ marginTop: 'auto', fontSize: 11.5, color: '#4f46e5', fontWeight: 800, borderTop: '1px dashed #e2e8f0', paddingTop: 8 }}>💰 {s.revenue}</div>
              </div>
            ))}
          </div>
        </section>
        ...
        <section id="core-bm" style={{ marginBottom: 32, scrollMarginTop: 70 }}>
          ...
          <div className="core-bm-grid">
            {/* Core BM 1. 급구 현장 공고 */}
            <div
              onClick={() => { track('bm_core_model_viewed', { model: 'urgent_job_posting' }); const bmObj = BM_DATA.find((b) => b.id === 'job-posting'); if (bmObj) openDrawer(bmObj, 2); }}
              style={{ border: '2px solid #bfdbfe', background: '#eff6ff', borderRadius: 16, padding: 20, cursor: 'pointer', display: 'flex', flexDirection: 'column', gap: 10 }}
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: 11.5, fontWeight: 900, background: '#1e40af', color: '#fff', padding: '3px 9px', borderRadius: 5 }}>최초 매출</span>
                <Badge badge={PRODUCT_STATUS_BADGE.pilot} />
              </div>
              <div style={{ fontSize: 18, fontWeight: 950, color: '#1e3a8a' }}>급구 현장 공고</div>
              <p style={{ margin: 0, fontSize: 13, color: '#1e40af', fontWeight: 650, lineHeight: 1.55, wordBreak: 'keep-all' }}>
                급하게 인력이 필요한 현장 운영사가 검증된 지역 기술자에게 공고를 빠르게 노출하고 건별 또는 패키지로 결제합니다.
              </p>
              <div style={{ background: '#fff', border: '1px solid #bfdbfe', borderRadius: 10, padding: '10px 12px', fontSize: 12, display: 'flex', flexDirection: 'column', gap: 5 }}>
                <div>💰 가격 가설: <strong>건당 3만~10만원</strong></div>
                <div>🎯 초기 고객: <strong>현장 운영사·협력사</strong></div>
                <div>🚀 Boost 옵션: <strong>지역·직무 타기팅, 긴급 알림, 상단 노출</strong></div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <Badge badge={EVIDENCE_BADGE.pilot} />
                  <span>전체 문의 24건 중 8곳 결제 의향 확인 (전체 문의 기준 33%, 2026.07 1주간)</span>
                </div>
              </div>
            </div>

            {/* Core BM 2. Partner Workspace Lite */}
            <div
              onClick={() => { track('bm_core_model_viewed', { model: 'partner_workspace' }); const bmObj = BM_DATA.find((b) => b.id === 'workspace-subscription'); if (bmObj) openDrawer(bmObj, 2); }}
              style={{ border: '2px solid #bbf7d0', background: '#f0fdf4', borderRadius: 16, padding: 20, cursor: 'pointer', display: 'flex', flexDirection: 'column', gap: 10 }}
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: 11.5, fontWeight: 900, background: '#166534', color: '#fff', padding: '3px 9px', borderRadius: 5 }}>반복매출</span>
                <Badge badge={PRODUCT_STATUS_BADGE.validating} />
              </div>
              <div style={{ fontSize: 18, fontWeight: 950, color: '#14532d' }}>Partner Workspace Lite</div>
              <p style={{ margin: 0, fontSize: 13, color: '#166534', fontWeight: 650, lineHeight: 1.55, wordBreak: 'keep-all' }}>
                매칭된 기술자와 팀의 출역·정산·평가 데이터를 관리하는 기업용 Workspace로 월 반복매출을 만듭니다.
              </p>
              <div style={{ background: '#fff', border: '1px solid #bbf7d0', borderRadius: 10, padding: '10px 12px', fontSize: 12, display: 'flex', flexDirection: 'column', gap: 5 }}>
                <div>💰 요금제 가설: <strong>월 4만~15만원</strong></div>
                <div>🎯 초기 고객: <strong>협력사·현장 운영사</strong></div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <Badge badge={EVIDENCE_BADGE.interview} />
                  <span>4곳 인터뷰 · 기능 PoC 요구 확인 (2026.07 B2B)</span>
                </div>
              </div>
            </div>

            {/* Differentiation Engine. 검증 프로필·팀 데이터 */}
            <div
              onClick={() => { track('bm_core_model_viewed', { model: 'trust_data' }); const bmObj = BM_DATA.find((b) => b.id === 'profile-access'); if (bmObj) openDrawer(bmObj, 2); }}
              style={{ border: '1.5px dashed #cbd5e1', background: '#f8fafc', borderRadius: 16, padding: 20, cursor: 'pointer', display: 'flex', flexDirection: 'column', gap: 10 }}
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: 11.5, fontWeight: 900, background: '#475569', color: '#fff', padding: '3px 9px', borderRadius: 5 }}>Trust Layer</span>
                <Badge badge={PRODUCT_STATUS_BADGE.validating} />
              </div>
              <div style={{ fontSize: 16.5, fontWeight: 950, color: '#334155' }}>검증 프로필·팀 데이터</div>
              <p style={{ margin: 0, fontSize: 13, color: '#475569', fontWeight: 650, lineHeight: 1.55, wordBreak: 'keep-all' }}>
                기술자의 경력·자격·안전교육·현장 이력을 구조화해 기업이 후보를 더 빠르고 신뢰성 있게 판단하도록 지원합니다. 급구 공고와 Workspace의 매칭 품질을 높이는 신뢰 데이터 계층이며, 별도 핵심 BM처럼 과금을 전면에 내세우지 않습니다.
              </p>
              <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 10, padding: '10px 12px', fontSize: 12, display: 'flex', flexDirection: 'column', gap: 5 }}>
                <div>💰 향후 과금 옵션: <strong>상세 검증정보 열람·후보 저장·연락 요청·대량 후보 조회·팀 단위 검증</strong></div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <Badge badge={EVIDENCE_BADGE.survey} />
                  <span>12건 상담 중 5곳 즉시 도입 의향 (2026.07 B2B)</span>
                </div>
              </div>
            </div>
          </div>
        ...
        {/* ── Field Pass Enterprise Expansion (v1.1 §6) ── */}
        <section id="field-pass" style={{ marginBottom: 32, scrollMarginTop: 70 }} onMouseEnter={() => track('bm_field_pass_viewed', { source_section: 'bm_page' })}>
          <div style={{ fontSize: 13, fontWeight: 850, color: '#4f46e5', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 12 }}>
            Enterprise Expansion — 대기업 진출·기술 해자
          </div>
          <div style={{ background: 'linear-gradient(135deg, #0f172a, #1e293b)', border: '1px solid #334155', borderRadius: 18, padding: 28 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 10, marginBottom: 14 }}>
              <div style={{ fontSize: 19, fontWeight: 950, color: '#ffffff' }}>MONO Field Pass — 산업현장 인력 신원·출입·근태 신뢰 인프라</div>
              <Badge badge={PRODUCT_STATUS_BADGE.prototype} />
            </div>
            <p style={{ fontSize: 13.5, color: '#cbd5e1', fontWeight: 600, lineHeight: 1.65, margin: '0 0 20px 0', wordBreak: 'keep-all', maxWidth: 760 }}>
              기술자의 신원, 자격, 교육, 현장 권한과 출입 기록을 하나로 연결하고, 출역 데이터를 정산과 경력으로 전환합니다. 기존 출입 시스템과 API로 연동하며, OTAC 기반 일회성 인증 적용을 통해 복제·재사용·대리 인증 위험을 줄이는 기술검증을 추진합니다.
            </p>

            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 16, marginBottom: 20 }}>
              <div>
                <div style={{ fontSize: 11.5, fontWeight: 800, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 8 }}>대상 고객</div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                  {['대형 건설사·원청', '반도체·플랜트 현장', '조선소·공장·물류센터', '협력사 관리 조직', '보안·안전·노무·IT/OT 조직'].map((c) => (
                    <span key={c} style={{ fontSize: 12.5, color: '#e2e8f0', fontWeight: 600 }}>· {c}</span>
                  ))}
                </div>
              </div>
              <div>
                <div style={{ fontSize: 11.5, fontWeight: 800, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 8 }}>수익모델</div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                  {[
                    ['현장 구독', '현장·협력사용 출입·출근 관리'],
                    ['구축·연동비', '기존 게이트·리더기·시스템 연동'],
                    ['사용자/인증 과금', '사용자 수 또는 인증 건수 기반'],
                    ['API·Gateway 라이선스', '기업 출입 시스템과 연동'],
                    ['리포트 구독', '협력사·자격·교육·안전·감사 리포트'],
                    ['Enterprise 계약', '다수 현장·법인 통합 운영'],
                  ].map(([name, desc]) => (
                    <div key={name} style={{ fontSize: 12.5, color: '#e2e8f0', fontWeight: 600 }}>
                      <strong style={{ color: '#a5b4fc' }}>{name}</strong> — {desc}
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div style={{ fontSize: 11.5, fontWeight: 800, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 8 }}>구성요소별 상태</div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 8, marginBottom: 20 }}>
              {[
                ['Field Pass 아키텍처', PRODUCT_STATUS_BADGE.prototype],
                ['QR 기반 출입', PRODUCT_STATUS_BADGE.poc_ready],
                ['NFC·카드 연동', PRODUCT_STATUS_BADGE.roadmap],
                ['OTAC 적용', PRODUCT_STATUS_BADGE.tech_review],
                ['PoC 범위 합의', PRODUCT_STATUS_BADGE.poc_planned],
                ['기술협력 문서 확보', PRODUCT_STATUS_BADGE.partnered],
                ['대기업 현장 적용', PRODUCT_STATUS_BADGE.enterprise_pilot],
                ['상용 운영', PRODUCT_STATUS_BADGE.live],
              ].map(([label, badge]) => (
                <div key={label as string} style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10, padding: '10px 12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
                  <span style={{ fontSize: 12.5, color: '#e2e8f0', fontWeight: 700 }}>{label as string}</span>
                  <Badge badge={badge as BadgeStyle} />
                </div>
              ))}
            </div>

            <div style={{ background: 'rgba(245,158,11,0.1)', border: '1px solid rgba(245,158,11,0.3)', borderRadius: 10, padding: '10px 14px', fontSize: 12, color: '#fcd34d', fontWeight: 650, lineHeight: 1.5 }}>
              ⚠️ 센스톤과 서면 합의 전에는 &quot;OTAC 적용 완료&quot;, &quot;센스톤 파트너&quot;와 같은 표현을 사용하지 않습니다. 현재 상태는 <strong>TECH REVIEW</strong>(기술 적합성 검토)입니다.
            </div>

            <button
              onClick={() => { track('bm_field_pass_cta_clicked', { source: 'field_pass_section' }); window.location.href = '#poc-cta'; }}
              style={{ marginTop: 18, padding: '11px 22px', background: '#ffffff', border: 'none', borderRadius: 10, fontSize: 13.5, fontWeight: 900, cursor: 'pointer', color: '#0f172a' }}
            >
              Field Pass PoC 문의하기 →
            </button>
          </div>
        ...
        <section
          ...
          onMouseEnter={() => track('founder_profile_clicked', { trigger: 'section_view' })}
        >
          <div style={{ fontSize: 13, fontWeight: 850, color: '#4f46e5', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 12 }}>
            Founder–Market Fit
          </div>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
            <a
              ...
              onClick={() => track('founder_profile_clicked', { link: 'security' })}
            ...
            <a
              ...
              target="_blank" rel="noopener noreferrer"
              onClick={() => track('founder_profile_clicked', { link: 'trive_team' })}
              style={{ fontSize: 13, fontWeight: 800, color: '#4f46e5', textDecoration: 'none', border: '1px solid #c7d2fe', padding: '8px 16px', borderRadius: 10 }}
            >
              T-rive Team 보기 ↗
            </a>
        ...
        {/* ── Team Readiness (v1.1 §10) ── */}
        <section style={{ background: '#ffffff', border: '2px solid #cbd5e1', borderRadius: 16, padding: 28, marginBottom: 32, boxShadow: '0 4px 20px rgba(0,0,0,0.02)' }} onMouseEnter={() => track('team_readiness_viewed', {})}>
          <div style={{ fontSize: 13, fontWeight: 850, color: '#4f46e5', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 4 }}>
            Team Readiness
          </div>
        ...
        {/* ── Regulatory Readiness (v1.1 §24, §26) ── */}
        <section style={{ background: '#ffffff', border: '2px solid #cbd5e1', borderRadius: 16, padding: 28, marginBottom: 32, boxShadow: '0 4px 20px rgba(0,0,0,0.02)' }}>
          <div style={{ fontSize: 13, fontWeight: 850, color: '#4f46e5', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 4 }}>
        ...
        {/* ── PoC CTA (v1.1 §4 Section 1 CTA + §18 PilotCTA) ── */}
        <section id="poc-cta" style={{ scrollMarginTop: 70, marginTop: 8, background: 'linear-gradient(135deg, #0f172a, #1e1b4b)', borderRadius: 20, padding: '36px 28px', textAlign: 'center' }}>
          <div style={{ fontSize: 20, fontWeight: 950, color: '#ffffff', marginBottom: 8 }}>PoC·기술 제휴 문의</div>
          <p style={{ fontSize: 13.5, color: 'rgba(226,232,240,0.85)', fontWeight: 600, lineHeight: 1.6, maxWidth: 620, margin: '0 auto 22px', wordBreak: 'keep-all' }}>
            급구 공고 파일럿, Workspace PoC, Field Pass PoC, 기술·사업 제휴 문의를 받고 있습니다. 아래 연락처로 기업명·담당자·문의 분야를 남겨주시면 담당자가 확인 후 회신드립니다.
          </p>
          <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: 10 }}>
            <a
              href="mailto:yunhyeok@t-rive.com?subject=MONO%20PoC%20문의"
              onClick={() => track('bm_poc_cta_clicked', { source: 'poc_cta_section' })}
              style={{ padding: '13px 26px', background: '#ffffff', borderRadius: 12, fontSize: 14, fontWeight: 900, color: '#111827', textDecoration: 'none' }}
            >
              PoC 문의 메일 보내기 →
            </a>
            <a
              href="mailto:yunhyeok@t-rive.com?subject=MONO%20기술·사업%20제휴%20문의"
              onClick={() => track('bm_contact_cta_clicked', { partner_type: 'unspecified' })}
              style={{ padding: '13px 26px', background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.25)', borderRadius: 12, fontSize: 14, fontWeight: 800, color: '#ffffff', textDecoration: 'none' }}
            >
              기술·사업 제휴 문의
            </a>
          </div>
