...
if '<div class="flowchart-wrapper' in html:
    html = re.sub(r'<div class="flowchart-wrapper.*?<!-- FLOWCHART_END -->', '<!-- FLOWCHART_PLACEHOLDER -->', html, flags=re.DOTALL)
    # also remove styles if we injected them
    html = re.sub(r'<style>\s*/\* Flowchart Colors.*?</style>', '', html, flags=re.DOTALL)

styles = """
<style>
  /* Flowchart Colors & Layout v3 */
  .flowchart-wrapper {
    width: 100%;
    position: relative;
    overflow: hidden;
  ...
  .flowchart-board {
    ...
    /* transform: scale() will be set by JS */
  }
...
flowchart_html = """
<div class="flowchart-wrapper mt-10" id="mono-flowchart">
<div class="flowchart-board" id="mono-board">
...
<!-- Flowchart Resize Script -->
<script>
  function resizeFlowchart() {
    const wrapper = document.getElementById('mono-flowchart');
    const board = document.getElementById('mono-board');
    if(wrapper && board) {
      const w = wrapper.clientWidth;
      const scale = w / 1600;
      board.style.transform = `scale(${scale})`;
      wrapper.style.height = `${1050 * scale}px`;
    }
  }
  window.addEventListener('resize', resizeFlowchart);
  document.addEventListener('DOMContentLoaded', resizeFlowchart);
...
with open('web/public/strategy.html', 'w', encoding='utf-8') as f:
    f.write(html)
print("Injected flowchart v3 with robust JS scaling and fixed arrows.")
