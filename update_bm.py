...
# 1. Update the one-line definition
old_desc = r'<p class="body-lg text-ink-800 font-bold max-w-4xl leading-relaxed" style="word-break: keep-all;">"기술자의 경험은 신뢰 데이터가 되고, 신뢰 데이터는 더 좋은 일자리와 금융 기회, 그리고 미래 기술의 기반이 됩니다."</p>'
new_desc = '<p class="body-lg text-ink-800 font-bold max-w-4xl leading-relaxed" style="word-break: keep-all;">"MONO는 기술자의 근무 경험을 신뢰 데이터로 축적하고, 이를 일자리·금융·교육·복지·미래 자산으로 연결하는 기술자 생애주기 플랫폼입니다."</p>'
if old_desc in html:
    html = html.replace(old_desc, new_desc)

# 2. Rebuild the Grid
new_grid = """<div class="grid lg:grid-cols-3 gap-6 mb-12 reveal stagger">
      <!-- 1. Core BM -->
...
# Replace the grid
html = re.sub(r'<div class="grid lg:grid-cols-3 gap-6 mb-12 reveal stagger">.*?</div>\n    </div>\n\n<!-- ============================== S6', new_grid + '\n    </div>\n</section>\n\n<!-- ============================== S6', html, flags=re.DOTALL)

with open('public/pitch.html', 'w', encoding='utf-8') as f:
    f.write(html)
print("BM section updated.")
